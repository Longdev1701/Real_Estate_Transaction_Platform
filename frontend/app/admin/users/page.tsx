...
import { AdminShell, NeonCard } from "@/components/admin/AdminShell";
import {
  getAdminUsers,
  updateAdminUser,
  type AdminUser,
...
const makeUsername = (user: AdminUser) =>
  `@${user.email.split("@")[0]?.replace(/[^a-zA-Z0-9_.-]/g, "").slice(0, 18) || user.id.slice(-6)}`;

const getActivity = (user: AdminUser) => {
  ...
  if (minutes <= 60) return { label: `Last seen ${minutes}m ago`, tone: "away" as const };
  return { label: `Offline ${Math.max(1, Math.floor(minutes / 60))}h ago`, tone: "offline" as const };
};
    ...
    >
      <div className="space-y-4">
        ...
        <section className="flex flex-wrap gap-2">
          <KpiChip icon={Users} label="Tổng User" value={counts.total} tone="blue" />
          <KpiChip icon={ShieldCheck} label="Đang hoạt động" value={counts.active} tone="green" />
          <KpiChip icon={Ban} label="Đã khóa" value={counts.banned} tone="red" />
          <KpiChip icon={Shield} label="Admin" value={counts.admins} tone="violet" />
          <KpiChip icon={UserCog} label="Người dùng" value={counts.users} tone="slate" />
        </section>
        ...
        <section className="flex flex-wrap gap-2">
          {[
            ...
            { key: "ACTIVE" as const, label: "Đang hoạt động", count: counts.active },
        ...
        <NeonCard className="p-3">
          <div className="grid items-center gap-2 xl:grid-cols-[minmax(300px,1.5fr)_170px_170px_170px_auto]">
        ...
        <NeonCard className="overflow-hidden">
          <div className="grid grid-cols-[minmax(220px,1.25fr)_minmax(180px,1fr)_110px_135px_135px_110px_110px_110px] border-b border-white/10 px-3 py-3 text-xs font-semibold uppercase tracking-wide text-gray-400">
            <div>User</div>
            <div>Email</div>
            <div>Vai trò</div>
            <div>Hoạt động</div>
          ...
          <div className="min-h-[52vh] divide-y divide-white/5">
            {isLoading && (
              <div className="px-5 py-10 text-center text-sm text-gray-400">Đang tải người dùng...</div>
            )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 px-5 py-3 text-sm text-gray-300">
            <span>
              Hiển thị {startIndex} - {endIndex} của {formatNumber(data?.meta.total ?? 0)} người dùng
            </span>
...
function UserRow({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="grid w-full grid-cols-[minmax(220px,1.25fr)_minmax(180px,1fr)_110px_135px_135px_110px_110px_110px] items-center px-3 py-3 text-left text-sm text-gray-200 transition hover:bg-blue-500/[0.04]"
    >
      ...
      <div className="truncate whitespace-nowrap pr-3 text-gray-300">{user.email}</div>
      <RoleBadge role={user.role} />
      <ActivityBadge label={activity.label} tone={activity.tone} />
      <div className="flex items-center gap-1 truncate whitespace-nowrap text-xs font-semibold text-gray-300">
        <p>🏠 {formatNumber(user._count.posts)}</p>
        <p>💬 {formatNumber(user._count.comments)}</p>
        <p>❤️ {formatNumber(user._count.savedPosts)}</p>
      </div>
      <div className="whitespace-nowrap text-gray-300">{formatDate(user.createdAt)}</div>
      ...
      <aside className="h-full w-full max-w-[450px] overflow-y-auto border-l border-blue-300/20 bg-[#061225]/95 p-5 shadow-[0_0_60px_rgba(37,99,235,0.22)]">
        ...
        <NeonCard className="p-5">
          ...
          <div className="mt-5 grid gap-3 text-sm">
            ...
            <InfoRow icon={Phone} label="SĐT" value={user.phone || "Chưa cập nhật"} />
            <InfoRow icon={Shield} label="Vai trò" value={roleLabels[user.role]} />
        ...
        <NeonCard className="mt-4 p-4">
          ...
          <div className="space-y-2">
            {user.posts.length ? (
              user.posts.map((post) => (
                <div key={post.id} className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
                  <p className="line-clamp-1 text-sm font-semibold text-white">{post.title}</p>
                  <p className="mt-1 text-xs text-gray-500">{formatDate(post.createdAt)} • {post.status}</p>
                </div>
              ))
            ) : (
              <p className="text-sm text-gray-400">Người dùng chưa có bài đăng.</p>
            )}
          </div>
        </NeonCard>
...
function KpiChip({
  ...
  const colors = {
    ...
    slate: "border-white/10 bg-white/[0.04] text-gray-200",
  };

  return (
    <div className={`inline-flex h-10 items-center gap-2 rounded-full border px-4 text-sm font-semibold ${colors[tone]}`}>
...
function UserAvatar({ user, size }: { user: AdminUser; size: "md" | "lg" }) {
  const className = size === "lg" ? "h-20 w-20 text-2xl" : "h-11 w-11 text-sm";

  return user.avatarUrl ? (
    <img src={user.avatarUrl} alt={user.fullName} className={`${className} shrink-0 rounded-full border border-blue-300/25 object-cover`} />
  ) : (
    <div className={`${className} grid shrink-0 place-items-center rounded-full border border-blue-300/25 bg-blue-500/15 font-bold text-blue-100`}>
...
function RoleBadge({ role }: { role: AdminUser["role"] }) {
  ...
  return (
    <span className={`inline-flex w-fit items-center gap-1.5 whitespace-nowrap rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${isAdmin ? "border-violet-400/30 bg-violet-500/10 text-violet-300" : "border-white/10 bg-white/[0.04] text-gray-300"}`}>
...
function StatusBadge({ status }: { status: AdminUser["status"] }) {
  ...
  return (
    <span className={`inline-flex w-fit items-center gap-1.5 whitespace-nowrap rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${active ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-300" : "border-red-400/30 bg-red-500/10 text-red-300"}`}>
...
function ActivityBadge({ label, tone }: { label: string; tone: "online" | "away" | "offline" }) {
  const colors = {
  ...
  return (
    <span className={`inline-flex items-center gap-2 whitespace-nowrap text-xs font-semibold ${colors[tone].split(" ")[1]}`}>
